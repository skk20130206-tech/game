...
게임 시작 후 이동 → 자원 채굴 → 스캔 → 집 건설 → 생명체 교류 → 이륙 → 다른 행성 착륙의 7단계를 안내합니다. 실제 성공한 행동으로만 진행하며, 미리 끝낸 행동도 인정합니다. 목표 위치 표시, 필요한 조작 버튼 강조, 안내 접기와 건너뛰기를 지원합니다. `?` 조작 방법의 **튜토리얼 다시 시작**으로 기존 탐험 기록을 유지하며 반복할 수 있습니다.

진행은 현재 브라우저의 `justgame-tutorial-v2`에 저장합니다. 새 탐험 또는 저장 파일 가져오기 시 안내를 처음부터 시작하며, 우주에서 시작해도 착륙 경로를 안내합니다. 원래 플레이어와 렌더러를 유지합니다.
