...
import playerTextureBase64 from "./assets/player-k17.webp.txt";
...
const BUILD="justgame-k17-motion-20260920-v16";
...
export default {
  async fetch(request){
    ...
    if(path==="/assets/player-k17.webp"){
      const bytes=Uint8Array.from(atob(playerTextureBase64.trim()),c=>c.charCodeAt(0));
      return new Response(bytes,{headers:{"content-type":"image/webp","cache-control":"public, max-age=86400",...securityHeaders}});
    }
    ...
    if(path==="/health"){
      try{
        ...
        return new Response(JSON.stringify({ok:true,build:BUILD,rendering:"WebGL 3D",modelSystem:"PioneerEX7-Mesh-v4",spaceshipModel:"Terralink Pioneer EX-7",spaceshipStyle:"heavy industrial survey lander",playerModel:"K-17 articulated reference character",playerAsset:"/assets/player-k17.webp",playerRigBones:16,interactionAnimations:true,tutorial:true,modelDownload:"/models/pioneer-ex7.glb",external3DDependency:false,immersive:true,creatureCare:true,companions:true,relics:15,htmlLength:html.length,creatures:11,mystic:true,startScreen:true,share:true,adsenseReviewReady:true,adsenseClient:ADSENSE_CLIENT,adsenseSnippetOnContentPages:true,gameRoute:"/play",contentPages:["/","/guide","/about","/updates","/privacy","/terms","/contact"],playAds:false,preApprovalAdPlaceholders:false}),{headers:{"content-type":"application/json; charset=UTF-8","cache-control":"no-store",...securityHeaders}});
      }catch(error){
