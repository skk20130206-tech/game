...
const BUILD = "server-gunzip-20260912-v1";
let gameHtmlPromise;

async function loadGameHtml() {
  if (b64.length !== 47024) {
    throw new Error(`ORBIT data length mismatch: expected 47024, got ${b64.length}`);
  }
  ...
  const decompressed = new Response(bytes)
    .body
    .pipeThrough(new DecompressionStream("gzip"));
  const html = await new Response(decompressed).text();

  if (!html.toLowerCase().includes("<!doctype html") || !html.includes("ORBIT")) {
    throw new Error(`ORBIT HTML validation failed (${html.length} chars)`);
  }
  return html;
}
